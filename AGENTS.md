...
- Nginx routes hostnames to separate PHP-FPM services (docker/nginx/vhost.conf): api.yii2-starter-kit.localhost -> api, backend.yii2-starter-kit.localhost -> backend, yii2-starter-kit.localhost -> frontend, storage.yii2-starter-kit.localhost -> storage.
...
- Maintenance mode is bootstrapped in frontend/api (common/components/maintenance/Maintenance.php) and toggled by APP_MAINTENANCE or key storage flags (both app configs currently read frontend.maintenance).
...
## MVC & Project conventions (important)
...
### View (Passive Views)
- Views must stay passive: do not execute queries/relation loading/request reads in view files.
- Prepare and pass all required data strictly from the controller or service layer.

### Controller (Lean Controllers)
- Add/adjust routes in app `_urlManager.php` files first; avoid hiding routing behavior inside controllers.
- Controllers should only be responsible for receiving requests, invoking the appropriate Models/Services, and returning the correct View or JSON response. Keep business logic out of controller actions.

## AI and MCP references
- Root AI guidance is this file (`AGENTS.md`); `README.md` is general project documentation.
...
- `codechap/yii2-ai-boost` is installed in `composer.json` (`require-dev`) and provides `boost/*` console tooling.
- MCP config is optional and local: `console/.mcp.json` is gitignored (`console/.gitignore`), so create/verify it per machine (PHP binary path + `console/yii` path) before relying on MCP.

## Additional Reading
- [Yii2 Boost Rules](console/.cursor/rules/yii2-boost.mdc): Comprehensive Yii2 coding standards and patterns.
- [AI Guidelines](console/.ai/guidelines): Detailed domain-specific guides (Auth, DB, Mailer, etc.).