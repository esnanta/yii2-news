...
use Yii;
use \backend\models\base\AccountPayable as BaseAccountPayable;
...
/**
 * This is the model class for table "tx_account_payable".
 */
class AccountPayable extends BaseAccountPayable
{
    /**
     * @inheritdoc
     */
    public function rules()
    {
        return [
            ...
            [['invoice'], 'unique', 'targetAttribute' => ['invoice'], 'message' => 'Invoice sudah ada dalam database.'], //ADD
            
            [['staff_id', 'invoice', 'date_issued'], 'required'],
            [['staff_id', 'date_issued', 'created_at', 'created_by', 'updated_at', 'updated_by', 'is_deleted', 'deleted_at', 'deleted_by', 'verlock'], 'integer'],
            [['description'], 'string'],
    ...
    public function beforeSave($insert) {
        ...
        if ($this->isNewRecord) {
            ...
            $this->title        = Counter::getSerialNumber(Counter::CODE_OF_ACCOUNT_PAYABLE);   
        }
    ...
    public function getUrl(){
        return Yii::$app->getUrlManager()->createUrl(['account-payable/view', 'id' => $this->id, 'title' => $this->title]);
    }    
