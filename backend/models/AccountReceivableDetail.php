...
use Yii;
use \backend\models\base\AccountReceivableDetail as BaseAccountReceivableDetail;
use common\helper\Helper as Helper;
use yii\behaviors\BlameableBehavior;
use yii\behaviors\TimestampBehavior;
use yii\db\ActiveRecord;
/**
 * This is the model class for table "tx_account_receivable_detail".
 */
class AccountReceivableDetail extends BaseAccountReceivableDetail
{
    ...
    public function rules()
    {
        return [
            ...
            [['description_detail'], 'safe'],
                        
            [['account_id', 'amount'], 'required'],
            [['account_receivable_id', 'account_id', 'created_at', 'updated_at', 'created_by', 'updated_by', 'is_deleted', 'deleted_at', 'deleted_by', 'verlock'], 'integer'],
            [['amount'], 'safe'], //CHANGE TO SAVE
            [['commentary'], 'string'],
            //[['invoice'], 'default', 'value' => date('y')],
            [['invoice'], 'string', 'max' => 20],
            //[['invoice'], 'unique', 'targetAttribute' => ['invoice'], 'message' => 'Invoice must be unique.'],
    ...
    public function attributeLabels()
    {
        return [         
            'id'                    => 'ID',
            'account_receivable_id' => Yii::$app->params['Attribute_AccountReceivable'],
    ...
    public function beforeSave($insert) {
        ...
        if ($this->isNewRecord) {
            $this->invoice          = date('y',$this->accountReceivable->date_issued).$this->invoice;             
        }         
